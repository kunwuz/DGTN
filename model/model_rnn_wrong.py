...
class Embedding2Score(nn.Module):
    def __init__(self, hidden_size, n_node, using_represent="comb", item_fusing=False):
...
class NARM(nn.Module):
    ...
    def padding(self, item_embs, seq_lens):
        padded = []
        ...
        for item_emb, seq_len in zip(item_embs, seq_lens):
            if item_emb.size(0) < max_len:
                pad_vec = torch.zeros(max_len - item_emb.size(0), self.hidden_size)
                pad_vec = pad_vec.to('cuda')
                item_emb = torch.cat((item_emb, pad_vec), dim=0)
            padded.append(item_emb.unsqueeze(dim=0))
        padded = torch.cat(tuple(padded), dim=0)
        return padded, masks
    ...
    def forward(self, item_embs, seq_lens):
        item_embs, masks = self.padding(item_embs, seq_lens)
...
class GraphModel(nn.Module):
    ...
    def forward(self, data):
        if self.item_fusing:
            ...
            h_s = self.narm.forward(intra_item_emb, data.sequence_len)
            ...
            # final_s = self.narm.forward(inter_item_emb, data.sequence_len)
            h_group = self.narm.forward(inter_item_emb, data.sequence_len)
